...
LastEditTime: 2025-01-10 11:29:26
...
def save_avi(visual_path, save_path):
    ...
    plt_plot_fov = mlab.points3d(
        ...
        # fov_voxels[:, 3],
        colormap="viridis",
        scale_factor=voxel_size - 0.05*voxel_size, #voxel_size - 0.05*voxel_size, # 0.01
        mode="cube",
        opacity=1.0,
        vmin=0,
        vmax=19,
    ...
    # 俯视图：
    # mlab.view(azimuth=180.0, elevation=0.0, distance=62) #, focalpoint=[pc_range[0], pc_range[3], pc_range[1], pc_range[2]])
    # 45度俯视图
    # mlab.view(azimuth=-45.0, elevation=-45.0, distance=60)
    # 前视相机
    # mlab.view(azimuth=90.0, elevation=75.0, distance=52)
    # 后视相机
    mlab.view(azimuth=-90.0, elevation=75.0, distance=40, focalpoint=[0.5, -2, 0.5])
    # mlab.move([0.5], [0.5], [-2.4+0.5])
